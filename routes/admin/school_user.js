...
router.get('/', async(ctx)=>{
    //ctx.body = '用户';
    var result = await DB.find('t_school_user',{});
   console.log(result);
    await ctx.render('admin/school_user/list',{
        list: result
    });
})

router.get('/add', async(ctx)=>{
    await ctx.render('admin/school_user/add');
})
...
router.post('/doAdd',async (ctx)=>{
    ...
    if(!/^\w{4,20}/.test(username)){
        await ctx.render('admin/error',{
            ...
            redirect: ctx.state.__HOST__ + '/admin/school_user/add'
        })
        ...
        if(findResult.length > 0){
            ...
            ctx.redirect(ctx.state.__HOST__ + '/admin/school_user');
        }
    }
...
router.get('/edit', async(ctx)=>{
    // ctx.body = '编辑用户';
    var id = ctx.query.id;
    var result = await DB.find('t_school_user',{"_id": DB.getObjectId(id)});
    await ctx.render('admin/school_user/editor.html',{
        list:result[0]
    });
})
    ...
    try{
        ...
        if(password != ''){     // 有修改
            if(password.length < 6){
                await ctx.render('admin/error',{
                    message: ' 密码不能少于6位',
                    redirect: ctx.state.__HOST__ + '/admin/school_user/edit?id=' + id
                })
            } else if(password != rpassword){
                await ctx.render('admin/error',{
                    message: '密码和确认密码不一致',
